...
class RaspberryPi:
        ...
        while True:
            ...
            if command.startswith(stm32_prefixes):
               ...
               # else:
                   ...
                   # rem = number - 99
                   # command1 = precom + "99"
                   # command2 = precom + str(rem)
                   # command3 = '['+str(command1) + ', ' + str(command2) + ']'
                   # self.stm_link.send(command1)
            #        self.stm_link.send(command2)
                    # Wait for acknowledgment (ACK) from STM32
                    #while(self.wait_for_acknowledgment())
                   # time.sleep(4)
                    #if not ack_received:
                    # Log an error if acknowledgment is not received
                    #    self.logger.error(f"No acknowledgment received for command: {command}")
    ...
    def snap_and_rec(self, obstacle_id: str) -> None:
        ...
        filename = f"{int(time.time())}_{obstacle_id}_L.jpg"
        ...
        # Test grab image id only (Right: 38, Left: 39)
        self.logger.info(results['image_id'])
